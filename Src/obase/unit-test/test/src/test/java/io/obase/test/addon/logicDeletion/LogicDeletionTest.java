...
public class LogicDeletionTest {
    ...
    public static void beforeAll() {
        ...
        for (var dataSource : TestCaseSourceConfigurationManager.getDataSources()) {
            ...
            //销毁可能的冗余数据
            context.createSet(LogicDeletion.class).delete(p -> p.getIntNumber() > 0, LogicDeletion.class);

            //添加新对象 一半删除一半没删除
            for (var i = 1; i < 21; i++) {
                var logicDeletion = new LogicDeletion();
    ...
    public void queryTest(EDataSource dataSource) {
        ...
        //新对象直接删除
        var newObject = new LogicDeletion();
        ...
        LogicDeletionExtensions.removeLogically(context.createSet(LogicDeletion.class), newObject);

        context = ContextUtils.createAddonContext(dataSource);
        //无条件查询
        var list = context.createSet(LogicDeletion.class).toList();
        ...
        for (var logicDeletion : list) {
            ...
            if (logicDeletion.getString().contains("1")) {
                LogicDeletionExtensions.removeLogically(context.createSet(LogicDeletion.class), logicDeletion);
            }
        }
        context.saveChanges();

        //查询
        list = context.createSet(LogicDeletion.class).toList();

        //有4个
        assertEquals(4, list.size());

        //测试直接逻辑删除
        LogicDeletionExtensions.deleteLogically(context.createSet(LogicDeletion.class), p -> p.getIntNumber() <= 5, LogicDeletion.class);
        ...
        //测试恢复
        LogicDeletionExtensions.recoveryLogically(context.createSet(LogicDeletion.class), p -> p.getIntNumber() >= 0, LogicDeletion.class);
