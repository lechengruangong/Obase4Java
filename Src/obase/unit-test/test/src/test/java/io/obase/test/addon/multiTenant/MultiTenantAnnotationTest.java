...
import io.obase.addon.test.domain.multi.tenant.MultiTenantSchoolAnnotation;
import io.obase.addon.test.domain.multi.tenant.MultiTenantTeacherAnnotation;
...
public class MultiTenantAnnotationTest {
    ...
    public static void beforeAll() {
        ...
        for (var dataSource : TestCaseSourceConfigurationManager.getDataSources()) {
            ...
            //销毁所有可能的冗余对象
            context.createSet(MultiTenantSchoolAnnotation.class).delete(p -> p.getSchoolId() > 0, MultiTenantSchoolAnnotation.class);
            context.createSet(MultiTenantTeacherAnnotation.class).delete(p -> p.getTeacherId() > 0, MultiTenantTeacherAnnotation.class);
            ...
            var teacher = new MultiTenantTeacherAnnotation();
            teacher.setName("某老师");
    ...
    public void queryTest(EDataSource dataSource) {
        ...
        var teacher = context.createSet(MultiTenantTeacherAnnotation.class).include(p -> p.getSchool()).findFirst().orElse(null);
        //校验取出的值
        ...
        //此处应无法查询出对象
        teacher = context.createSet(MultiTenantTeacherAnnotation.class).include(p -> p.getSchool()).findFirst().orElse(null);
        //无法查出
        assertNull(teacher);
    }
}
