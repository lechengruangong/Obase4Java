...
│　作   者：Obase开发团队
│　版权所有：武汉乐程软工科技有限公司
│　创建时间：2025-12-30 16:28:11
└──────────────────────────────────────────────────────────────┘
*/
package io.obase.core.query.oop;

import io.obase.core.query.AnyOp;
 ...
 * Any测定运算执行器
 */
public class AnyExecutor extends OopExecutor {

    /**
     * 要执行的查询运算
     */
    private final AnyOp op;
    ...
    public void execute(OopContext oopContext) {
        if (oopContext.getResult() instanceof Iterable) {
            ...
            oopContext.setResult(list.stream().anyMatch((Predicate<? super Object>) ExpressionDelegates.getInstance().get(this.op.getPredicate())));
        } else {
