 ...
 * 时间字段设值器
 */
public class TimeSpanFieldSetter extends FieldSetter<LocalTime> {
    /**
     * 构造字段设值器
     *
     * @param field     字段名称
     * @param localTime 字段值
     */
    public TimeSpanFieldSetter(String field, LocalTime localTime) {
        super(field, localTime);
    }
     ...
     * @param localTime 字段值
     */
    public TimeSpanFieldSetter(String source, String field, LocalTime localTime) {
        super(source, field, localTime);
    }
     ...
     * @param localTime 字段值
     */
    public TimeSpanFieldSetter(Field field, LocalTime localTime) {
        super(field, localTime);
    }
    ...
    public String toString(EDataSource sourceType) {
        LocalTime time = this.getValue();
        return this.field.toString(sourceType) + "='" + time.format(DateTimeFormatter.ofPattern("HH:mm:ss.SSS")) + "'";
    }
    ...
    public String toString(ObjectReferencePack<DataParameter> parameters, EDataSource sourceType, IParameterCreator creator) {
        LocalTime time = this.getValue();
        String valueStr = time.format(DateTimeFormatter.ofPattern("HH:mm:ss"));
    ...
    public String toString(ObjectReferencePack<DataParameter> parameters, ObjectReferencePack<String> field, EDataSource sourceType, IParameterCreator creator) {
        ...
        LocalTime time = this.getValue();
        String valueStr = time.format(DateTimeFormatter.ofPattern("HH:mm:ss.SSS"));
    ...
    protected String getParameters(ObjectReferencePack<DataParameter> parameters, EDataSource sourceType, Object valueStr,
        ...
        if (sourceType == EDataSource.PostgreSql && aNull) {
            parameters.realValue.Value = java.sql.Timestamp.valueOf(LocalDate.now() + " " + parameters.realValue.Value.toString());
        }
        return parameter;
    }
}
