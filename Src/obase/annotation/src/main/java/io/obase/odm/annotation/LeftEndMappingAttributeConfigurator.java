 ...
 * 隐式关联左端标注属性配置器
 */
public class LeftEndMappingAttributeConfigurator extends MemberAttributeConfigurator {
     ...
     */
    public LeftEndMappingAttributeConfigurator(String keyAttribute, String targetField) {
    ...
    protected void configureAttribute(Property memberInfo, IAttributeConfigurator configurator) {
        throw new IllegalArgumentException("隐式关联左端标注不应调用此方法");
    }
    ...
    protected void configureAssociationReference(Property memberInfo, IAssociationReferenceConfigurator configurator) {
        ...
        if (configurator.upwardI() instanceof StructuralTypeConfiguration) {
            ...
            if (implicitAssociationConfig != null) {
                ...
                ends.stream().filter(p -> p.getEntityType().equals(memberInfo.getGetterMethod().getDeclaringClass())).findFirst()
